...
exports.BrainSerialConnection = void 0;
...
class MockSerialPort {
    constructor(options) {
        ...
        console.log(`[MOCK] Creating serial port ${this.path} at ${this.baudRate} baud`);
    }
    open(callback) {
        setTimeout(() => {
            this.isOpen = true;
    ...
    simulateData() {
        ...
        setTimeout(() => {
            ...
            this.eventHandlers.data.forEach((handler) => handler(Buffer.from(randomMessage)));
...
try {
    ...
    console.log(chalk_1.default.yellow("Native serialport module not available, switching to simulation mode"));
    console.log(chalk_1.default.cyan("In simulation mode, no real hardware connections will be made"));
    MOCK_MODE = true;
}
// Helper function for resolving binary paths when packaged with pkg
function getSerialportBinaryPath() {
    ...
    if (isPackaged) {
        ...
        return path.join(path.dirname(process.execPath), "serialport", "build", "Release");
    }
 ...
 */
class BrainSerialConnection {
    constructor() {
        this.port = null; // Using 'any' to support both real and mock ports
...
exports.BrainSerialConnection = BrainSerialConnection;
